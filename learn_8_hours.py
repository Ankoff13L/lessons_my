...
# a = [len(s) for s in ['cat', 'hello', 'goodbye']]
# print(a)


 
# def turbo(x):              # определяем функцию 'turbo' с пареметром 'x'
# #Далее идёт инструкция для функции, что она должна делать
#     print(x + 7)          # напечатать сумму 'x+7', когда станет известен 'x'
#     print('hello world')  # напечатать строку 'hello world'
#     return(x**2)          # вернуть результат возведения 'x' в квадрат. Т.е. записать 
# # в переменную 'y' (переменная, которая вызвала функцию) результат 'x' в квадрате.

# y = turbo(9) # переменная вызывающая функцию 'turbo' с аргументом 9 (значение для 'x')
# print(y)


# def turbo(a):   # a = [100, 200, 300]
#     s = 0       # создаём для цикла переменную 's', которая изначально имеет значение '0'
#     for x in a: # перебираем список и значение каждого элемента поочерёдно передаём 'x' # a = [100, 200, 300]
#         s += x  # поочерёдно значение каждого элемента прибавляем в 's'
#     return(s)   # после завершения цикла возвращаем сумму в 'primt()' для печати

# print(turbo([100, 200, 300])) # вызываем функцию 'turbo' и передаём ей
# # в качестве аргумента список из трёх чисел.
